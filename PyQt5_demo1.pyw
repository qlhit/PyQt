...
class MainWindow(QMainWindow, form13.Ui_MainWindow):
    #signal definition
    newdata = pyqtSignal()
...
#       two signals instead of one!
    ...
    def tree_response(self):
        ...
        elif key == '测量开始':self.on_StartMeasBtn_clicked(self.treeWidget.topLevelItem(1).child(0).text(1),self.treeWidget.topLevelItem(1).child(1).text(1),self.treeWidget.topLevelItem(1).child(2).text(1))
        elif key == '测量结束':self.on_StopMeasBtn_clicked()
...
#         # QMessageBox.information(self,"Info","AVS_Activate returned:  {0:d}".format(globals.dev_handle))
    ...
    @pyqtSlot()
    def on_StartMeasBtn_clicked(self,IntTimeEdt,NumAvgEdt,NumMeasEdt):
...
#         # QMessageBox.information(self,"Info","Received data")
        ...
        #一轮扫描结束
        if self.scans == len(self.data_time) or globals.stopscanning:
            ...
            t1 = threading.Thread(target=self.p1.clearPlots())
            t1.setDaemon(True)
            t1.start()

            # QMessageBox.information(self,'Info','绘制')
            for indexSI in range(len(self.data_wavelength)):
                # QMessageBox.information(self,'Info','绘制time vs spectrInt')
                self.p1.plot(self.data_time, self.data_colu_spectrInt[:, indexSI], pen=(random.randint(0,200),random.randint(0,200),random.randint(0,200)), clear=False)
                qApp.processEvents()
    ...
    def show_pca(self,file_name):
        ...
        self.pc1.plot(cur_sm[:,0],pen='red')
        self.pc1.plot(pre_sm[:,0],pen='blue')
        self.pc2.plot(cur_sm[:,1],pen='red')
        self.pc2.plot(pre_sm[:,1],pen='blue')
        self.pc3.plot(cur_sm[:,2],pen='red')
        self.pc3.plot(pre_sm[:,2],pen='blue')
        self.pc4.plot(cur_lm[0,],pen='red')
        self.pc4.plot(pre_sm[0,],pen='blue')
        self.pc5.plot(cur_lm[1,],pen='red')
        self.pc5.plot(pre_sm[1,],pen='blue')
        self.pc6.plot(cur_lm[2,],pen='red')
        self.pc6.plot(pre_sm[2,],pen='blue')
