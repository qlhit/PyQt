...
#         # QMessageBox.information(self,"Info","Received data")
    ...
    def t2(self):
        ...
        self.finish2.emit()
    # 启动定时器 时间间隔秒
    def timer_start1(self):
        print('计时器')

        timer = QtCore.QTimer(self)
        timer.timeout.connect(self.t1)
        timer.start(1)
        self.finish1.connect(timer.stop)
    # 启动定时器 时间间隔秒
    def timer_start2(self):
        print('计时器')
        timer = QtCore.QTimer(self)
        timer.timeout.connect(self.t2)
        timer.start(1)
        self.finish2.connect(timer.stop)
    @pyqtSlot()
...
#         # QMessageBox.information(self,"Info","Received data")
        ...
        if self.scans == len(self.data_time) or globals.stopscanning:
            ...
            #     qApp.processEvents()
            self.timer_start1()
            # dr = draw_runtime(self.p1,self.data_time,self.data_wavelength,self.data_colu_spectrInt,qApp)
            # dr.start()
            # print(dr.isFinished())
            # for indexWa in range(len(self.data_time)):
            #     # QMessageBox.information(self,'Info','wavlength vs spectrInt')
            ...
            #     time.sleep(1)
            #     qApp.processEvents()
            # dw = draw_wavel(self.p2,self.data_time,self.data_wavelength,self.data_colu_spectrInt,qApp)
            # dw.start()
            self.timer_start2()
            # print(dw.isFinished())
            self.set_xyline()
